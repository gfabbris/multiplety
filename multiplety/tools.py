...
def save_rixs(rixs,atomic_number,atom_label,gs_config,es_config,
    ...
    file = open(join(save_folder,fname),'w')
...
def load_xas(save_folder, fname):
    ...
    return pd.read_csv(join(save_folder,fname),comment='#',dtype=np.float64)
...
def load_rixs(save_folder, fname):
    ...
    rixs = pd.read_csv(join(save_folder,fname),comment='#',dtype=np.float64)
